...
# SPM = ImportCSV("MitagSPM",None)
# SPM = ImportCSV("MegiSPM",None)
...
Rmsf = pd.DataFrame() # "t", "t+1", "t+2", ...."t+11"
Rmsf["TPB"] = Ts["TPB"]
for i in range(1,24):
    Rmsf[f"TPBt+{i}"] = Ts["TPB"].shift(-i)
    # Rmsf["TPBt+11"] = Ts["TPB"].shift(-11)

# SMOmsf = pd.DataFrame() # "t", "t+1", "t+2", ...."t+11"
# SMOmsf["SMOutflow"] = Ts["SMOutflow"]
# for i in range(1,12):
#     SMOmsf[f"SMOutflowt+{i}"] = Ts["SMOutflow"].shift(-i)
#     # Rmsf["TPBt+11"] = Ts["TPB"].shift(-11)


FTmsf = pd.DataFrame() # "t", "t+1", "t+2", ...."t+11"
FTmsf["FTOutflow"] = Ts["FTOutflow"]
for i in range(1,24):
    FTmsf[f"FTOutflowt+{i}"] = Ts["FTOutflow"].shift(-i)

Tmsf = pd.DataFrame() # "t", "t+1", "t+2", ...."t+11"
Tmsf["Tide"] = Ts["Tide"]
for i in range(1,24):
    Tmsf[f"Tidet+{i}"] = Ts["Tide"].shift(-i)
...
for ty, starttime in zip(Ty, Starttime):
    ...
    for startPoint in range(len(event)):
        ...
        forcasting = Prediction(fitModel, NPara.ModelName, Xtest) #t+1
        F_Inv = Npr._InverseCol(forcasting) 
        ...
        # for i in range(time+1,len(X_test)-2):
            # R = [[t+1, t+2, t+3, t+4],[t1+1, t1+2, t1+3, t1+4]] 預報的pattern
            row = starttime+max([R, SMO, FT, T, WL])+startPoint
            #Mitag 代3+startPoint
            recursiveForsO = [Rmsf.iloc[row,i], 0.9*SMOmsf.iloc[startPoint,i], FTmsf.iloc[row,i], Tmsf.iloc[row,i],forcasting]
            recursiveFors = Npr.sc.transform([recursiveForsO])[0]
            new_x, new_y = msf1D(endTimeList = endTimeList, timeList = NPara.TStepList, Fors = recursiveFors, X_test = Xtest , Y_test = "" , forcasting=forcasting)
